...
case class Waiting(countDown: Int, startPosition: Point, numberOfParticles: Int, particlesColor: Color) extends Firework:
  ...
  def next: Firework =
    if countDown > 0 then
      ...
      Firework.next(Launched.init(startPosition, numberOfParticles, particlesColor))
end Waiting
...
case class Launched(countDown: Int, position: Point, direction: Angle, numberOfParticles: Int, particlesColor: Color) extends Firework:
  ...
  def next: Firework =
    if countDown > 0 then
      copy(position = Motion.movePoint(position, direction, Settings.propulsionSpeed))
      copy(countDown = countDown - 1)
    else
      Firework.next(Exploding.init(numberOfParticles, direction, position, particlesColor))
