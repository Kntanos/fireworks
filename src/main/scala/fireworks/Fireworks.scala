...
case class Launched(countDown: Int, position: Point, direction: Angle, numberOfParticles: Int, particlesColor: Color) extends Firework:
  ...
  def next: Firework =
    if countDown > 0 then
      ...
      val updatedCountDown = countDown - 1
      Launched(updatedCountDown, updatedPosition, direction, numberOfParticles, particlesColor)
    else
...
case class Exploding(countDown: Int, particles: Particles) extends Firework:
  ...
  def next: Firework =
    if countDown > 0 then
      val updatedParticles = particles.next
      val updatedCountDown = countDown - 1
      Exploding(updatedCountDown, updatedParticles)
    else
      Done
